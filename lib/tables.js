...
function changeCharacterCheckbox(selected, extension = "png") {
    ...
    if ($(selected).is(":checked")) {
        for (var k = 0; k <= currentGameChars[gameIndex][1]; k++) {
            ...
            /* wip for tooltip support - reqs bootstrap
            $("#char div:last-child").attr("title", `${allCharacters[gameId][k]}`);
            $("#char div:last-child").attr("data-toggle", "tooltip");
            $("#char div:last-child").tooltip();
            */
        }
    } else {
